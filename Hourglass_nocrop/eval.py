...
#     '/media/home_bak/ziqi/park/Hourglass_all/57heatmap.ckpt'))
...
#     file = os.path.join(
#         "/media/home_bak/ziqi/park/Hourglass_all/test_img", f)
#     img = cv2.imread(file)
#     total = 0
#     for i in range(4):
...
#             "/media/home_bak/ziqi/park/Hourglass_all/heatmap_gt_test", f)
...
# plt.ylabel('accuracy(%)')
# plt.show()
# plt.savefig("/media/home_bak/ziqi/park/Hourglass_all/accuracy.png")
...
import cv2
import numpy as np
from dataloader.dataloader import heatmap_Dataloader
import os
from net import KFSGNet
import matplotlib.pyplot as plt  # plt 用于显示图片
from heatmappy import Heatmapper
import matplotlib
matplotlib.use('Agg')
...
model.to(device)
model.load_state_dict(torch.load(
    '/media/home_bak/ziqi/park/Hourglass_nocrop/30heatmap.ckpt'))
...
def get_acc(path, y, y_hat, dis):
    f = os.path.basename(path)
    file = os.path.join(
        "/media/home_bak/ziqi/park/Hourglass_nocrop/test_img", f)
    save_img = os.path.join(
        "/media/home_bak/ziqi/park/Hourglass_nocrop/error_img", f)
    img = cv2.imread(file)
    total = 0
    for i in range(2):
        total += ((y[i][0]-y_hat[2*i])**2 + (y[i][1]-y_hat[2*i+1])**2)**0.5
...
for i, (data, gt, mask, item, imgPath, heatmaps_targets) in enumerate(test_loader):
    ...
    for k in range(len(imgPath)):
        ...
        point_path = os.path.join(
            "/media/home_bak/ziqi/park/Hourglass_nocrop/point", f.strip('.jpg')+'.txt')
        ...
        img_back_path = os.path.join(
            "/media/home_bak/ziqi/park/Hourglass_nocrop/test_img_back", f)
        gt_back_path = os.path.join(
            "/media/home_bak/ziqi/park/Ps_locate_dataset/PLD_BirdView_Training_TestSet_v1.0.7_All_3342/point", f.strip('.jpg')+'_OA.txt')
    ...
    for p in range(len(imgPath)):
        f = os.path.basename(imgPath[p])
        path_gt = os.path.join(
            "/media/home_bak/ziqi/park/Hourglass_nocrop/heatmap_gt_test", f)
        # heatmapper = Heatmapper()
        point = []
        point = [(int(all_peak_points[p][0][0]),
                  int(all_peak_points[p][0][1])),
...
plt.ylabel('accuracy(%)')
plt.show()
plt.savefig("/media/home_bak/ziqi/park/Hourglass_nocrop/accuracy3.png")
