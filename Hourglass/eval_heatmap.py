...
def get_acc(path, y, y_hat, dis, save_img):
    ...
    file = os.path.join(
        "/media/home_bak/ziqi/park/Hourglass/test_img", f)
    img = cv2.imread(file)
    total = 0
    for i in range(2):
    ...
    for p in range(21):
        for s in range(len(imgPath)):
            ...
            save_img = os.path.join(
                "/media/home_bak/ziqi/park/Hourglass/error_img", f)
...
plt.savefig("/media/home_bak/ziqi/park/Hourglass/accuracy.png")
