...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_prob = outputs["pred_classes"].flatten(0, 1).softmax(-1)  # [batch_size * num_queries, num_classes]
        out_bbox = outputs["pred_boxes"].flatten(0, 1)  # [batch_size * num_queries, 10]
        out_bbox_center=out_bbox[:,8:]
        out_bbox_points=out_bbox[:,:8]
        l=[]
        for i in range(self.batch_size):
            # print('targets',targets[i + 1].shape)
            a = torch.tensor([]).cuda() if targets[i + 1].shape[1] == 1 else torch.ones(targets[i + 1].shape[1]-1).cuda()
            b = torch.tensor([]).cuda() if targets[i + self.batch_size + 1].shape[1] == 1 else (torch.ones(targets[i +self.batch_size+1].shape[1]-1)*2).cuda()
        ...
        for i in range(self.batch_size):
            a=torch.tensor([]).cuda() if targets[i+1].shape[1]==1 else targets[i+1][0,1:]
            b=torch.tensor([]).cuda() if targets[i+self.batch_size+1].shape[1]==1 else targets[i+self.batch_size+1][0,1:]
            l.append(torch.cat((a,b)))
        ...
        ll=[torch.cdist(out_bbox_points,tgt[:,i,:].view(-1,8),p=1).unsqueeze(-1) for i in self.list]
        l2=torch.cat(ll,axis=-1)
        cost_bbox_points,_=torch.min(l2, dim=-1)
        C = self.cost_bbox_center * cost_bbox_center + self.cost_class * cost_class+self.cost_bbox_points*cost_bbox_points
        C = C.view(bs, num_queries, -1).cpu()
        sizes = [sum([v.shape[1] - 1, q.shape[1] - 1]) for v, q in
                 zip(targets[1:self.batch_size + 1], targets[1 + self.batch_size:])]
        indices = [linear_sum_assignment(c[i]) if sizes[i]!=0 else (np.array([-1]),np.array([-1])) for i, c in enumerate(C.split(sizes, -1))]
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]


def build_matcher(set_cost_bbox_points,set_cost_class,set_cost_bbox_center,batch_size):
