    ...
    for y in range(ymaps):
        for x in range(xmaps):
            if k >= nmaps:
                break
            if flag=='gth':
                # print('batch_joints',batch_joints[k][0].shape)
            ...
            if joints is not None:
                ...
                for ps in joints:
                    ...
                    for idx,joint in enumerate(ps_order):
                        ...
                        # joint[3]
                        # if joint_vis[0]:
                        cv2.circle(ndarr, (int(joint[0]), int(joint[1])), 2, colors[i], 2)
                        if idx==0:
                            ini_joint=joint
                        if idx>0:
                            # print('jjjjjj',int(joint),int(last_joint))
                            cv2.line(ndarr, (int(joint[0]),int(joint[1])),(int(last_joint[0]),int(last_joint[1])), GREEN , 3)
                        last_joint=joint
                        i += 1
                    cv2.line(ndarr, (int(ini_joint[0]),int(ini_joint[1])), (int(last_joint[0]),int(last_joint[1])), GREEN, 3)
                    center_x = x * width + padding + ps[-1, 0]
                    center_y = y * height + padding + ps[-1, 1]
                    cv2.circle(ndarr, (int(center_x), int(center_y)), 5, BLUE, 5)
            ...
            if joints_ign is not None:
                ...
                for ps in joints_ign:
                    ...
                    ps_order = set_order(ps[:4, :])
                    for idx,joint in enumerate(ps_order):
                        if joint[0]>384 or joint[0]<0:
                            raise ValueError
                        if joint[1] > 384 or joint[1] < 0:
                            raise ValueError
                        joint[0] = x * width + padding + joint[0]
                        joint[1] = y * height + padding + joint[1]
                        # joint[2]
                        ...
                        if idx>0:
                            # print('jjjjjj',int(joint),int(last_joint))
                            cv2.line(ndarr, (int(joint[0]),int(joint[1])),(int(last_joint[0]),int(last_joint[1])), RED , 3)
                        last_joint=joint
                        i += 1
                    cv2.line(ndarr, (int(ini_joint[0]),int(ini_joint[1])), (int(last_joint[0]),int(last_joint[1])), RED, 3)
                    center_x=x * width + padding + ps[-1,0]
                    center_y=y * height + padding + ps[-1,1]
                    cv2.circle(ndarr, (int(center_x), int(center_y)), 5, BLUE, 5)
            k = k + 1
