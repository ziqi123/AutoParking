...
def preprocessing(b_ind,
    ...
    joints = joints.copy()
    joints_ign=joints_ign.copy()



    if joints.shape[0]>1:

        ujoints  = np.zeros((num_roi, joints.shape[0], joints.shape[1]), dtype=np.float)
        ujoints[0] = joints.astype(float)

    else:
        ujoints=np.zeros((num_roi, 1, 8), dtype=np.float)
    if joints_ign.shape[0]>1:
        ujoints_ign=np.zeros((num_roi, joints_ign.shape[0], joints_ign.shape[1]), dtype=np.float)
...
def _resize_image(db,
    ...
    # print('ujoints',ujoints)
    ujoints=(ujoints/4)*1.5
    ujoints=add_center(ujoints)
    ujoints_ign=(ujoints_ign/4)*1.5
    ...
    for i in range(num_roi):
        ...
        for j in range(num_pslot):
            for k in range(5):
                ps_joints[j,k,:]= affine_transform(ps_joints[j,k, :], trans)
        ...
        for j in range(num_ign_pslot):
            for k in range(5):
                ps_joints_ign[j, k, :] = affine_transform(ps_joints_ign[j, k, :], trans)
        ...
        tgt_areas.append(((br[0] - tl[0]) / roi_size[1]) * ((br[1] - tl[1]) / roi_size[0]))
        ...
        tgt_joints_ign.append(torch.from_numpy(repeat_joints_ign.astype(np.float32)))
...
def add_center(joints):
    ...
    for joi in joints[0]:
        ...
        center=np.expand_dims((joi_p[0]+joi_p[1]+joi_p[2]+joi_p[3])/4,axis=0)
