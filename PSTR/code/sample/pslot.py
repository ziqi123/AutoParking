...
def kp_detection(db, k_ind, data_aug, debug, pose_debug=False):
    ...
    inputs         = np.zeros((batch_size, num_roi, 3, roi_size[0], roi_size[1]), dtype=np.float32)
    inmasks        = np.zeros((batch_size, num_roi, 1, roi_size[0], roi_size[1]), dtype=np.float32)
    ...
    for b_ind in range(batch_size):
        ...
        ps_joints,ps_joints_ign = db.detections(db_ind) # all in the raw coordinate
    ...
    if pose_debug:
        viz_inputs = torch.from_numpy(inputs.reshape((-1, 3, roi_size[0], roi_size[1])))
        used_tgt_joints     = torch.from_numpy(roi_size[0] * tgt_joints.reshape((-1, num_joints, 3)))  # 1 6 5 3
        used_tgt_joints_vis = torch.from_numpy(roi_size[0] * tgt_joints_vis.reshape((-1, num_joints, 3)))  # 1 6 5 3
