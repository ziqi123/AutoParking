...
import cv2
import numpy as np
from dataloader.dataloader_stackedHourglass_256 import heatmap_Dataloader
import os
from models.posenet_256 import PoseNet
import matplotlib
import math
...
model.load_state_dict(torch.load(
    '/media/home_bak/ziqi/park/stackedHourglass_256/10heatmaps_Adaptive_wing_loss2.ckpt'))
...
for i, (data, gt, mask, item, imgPath, heatmaps_targets) in enumerate(test_loader):
    ...
    # # 热图还原为坐标点
    all_peak_points = 4.0*get_peak_points(outputs[0].cpu().data.numpy())
    # heatmap_point = 4.0*get_peak_points(heatmaps_targets.cpu().data.numpy())
...
#     # # 标点
    ...
    #         point_384_gt_path = os.path.join(
    #             point_384_gt_dir, f.strip('.jpg')+'.txt')
    #         tmp = get_angle_acc(point_pred_384_path,
    #                             point_384_gt_path, pix)
