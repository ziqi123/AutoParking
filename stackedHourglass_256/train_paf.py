...
import os
from models.posenet_PAF_HRU import PoseNet
...
pafLoss = PAFLoss()
focal_loss = nn.BCEWithLogitsLoss(size_average=False)
for epoch in range(num_epochs):
    ...
    for i, (data, gt, mask, item, imgPath, heatmaps_targets, vectormap) in enumerate(train_loader):
        ...
        for j in range(8):
            ...
            # combined_loss.append(focal_loss(
            #     outputs['heatmaps'][:, j], heatmaps_targets))
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(),
                   '{}heatmaps_sigma3_1_nstack8_PAF_new3.ckpt'.format(epoch + 1))
