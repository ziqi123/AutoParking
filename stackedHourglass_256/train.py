...
#     '/media/home_bak/ziqi/park/stackedHourglass_256/10heatmaps_Adaptive_wing_loss.ckpt'))
...
#                                      milestones=[10, 20, 30, 40, 50, 60], gamma=0.5)
...
for epoch in range(num_epochs):
    ...
    for i, (data, gt, mask, item, imgPath, heatmaps_targets) in enumerate(train_loader):
        ...
        # print("len(heatmaps_targets)", len(heatmaps_targets))
        combined_loss = []
        for j in range(8):
            combined_loss.append(heatmapLoss(
                outputs[:, j], heatmaps_targets))
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(),
                   '{}heatmaps_FMSE.ckpt'.format(epoch + 1))
