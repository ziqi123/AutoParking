...
model.load_state_dict(torch.load(
    '/media/home_bak/ziqi/park/stackedHourglass_256/30heatmaps_sigma3_1_nstack8_PAF_new.ckpt'))
...
def get_angle_acc(point_pred_256_path, point_256_gt_path, pix):
    point_pred = np.loadtxt(point_pred_256_path)
    point_gt = np.loadtxt(point_256_gt_path)
    vec = [[0]*3] * 5
    vec_gt = [[0]*3] * 5
    for w in range(4):
    ...
    for pix in range(10):
        for c in range(len(imgPath)):
            ...
            point_pred_256_path = os.path.join(
                point_pred_256_dir, f.strip('.jpg')+'.txt')
            point_256_gt_path = os.path.join(
                point_256_gt_dir, f.strip('.jpg')+'_OA.txt')
