...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_pslots, indices2, num_pslots2):
        """Classification loss
        0:negative sample
        ...
        # tmp = src_logits.transpose(1, 2)
        # print('tmp', tmp.shape)
        # [16, 3, 2500]
        # print('target_classes', target_classes.shape)
        # [16, 2500]
        # print('target_classes', torch.max(target_classes[0]))
    ...
    def loss_class(self, outputs, targets, indices, num_pslots, indices2, num_pslots2):
        """Classification loss
        0:negative sample
        1:visible parking slots
        2:ignored parking slots
    ...
    def loss_box(self, outputs, targets, indices, num_pslots, indices2, num_pslots2):
        ...
        losses = {}
        losses['loss_box'] = (
            loss_bbox.sum()+loss_bbox_ign.sum()) / num_pslots2
        return losses

    def loss_joints(self, outputs, targets, indices, num_pslots, indices2, num_pslots2):
        """Compute the losses related to the parking slot location with the L1 regression loss.
           The target boxes are expected in format (p1_x,p1_y,p2_x,p2_y,p3_x,p3_y,center_x, center_y), normalized by the image size.
        """
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, indices2, num_boxes2, **kwargs):
        ...
        return loss_map[loss](outputs, targets, indices, num_boxes, indices2, num_boxes2, **kwargs)
    ...
    def forward(self, outputs, targets):
        ...
        for loss in self.losses:
            losses.update(self.get_loss(
                loss, outputs, targets, indices, num_pslots, indices2, num_pslots2))
