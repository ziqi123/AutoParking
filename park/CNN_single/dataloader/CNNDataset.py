...
class CNNDataset(Dataset):
    def __init__(self, ds_dir, setname='train', transform=None, norm_factor=256, rgb2gray=False, resize=True):
        ...
        if setname == 'train':
            data = []
            gt = []
            train_list = '/media/home_bak/ziqi/park/CNN_single/dataset/train.txt'
            f = open(train_list)
            for line in f:
                line_data = line.strip('\n')
                line_gt = line_data.replace(
                    'pic_crop', 'point_crop').replace('.jpg', '_OA.txt')
        ...
        if setname == 'val':
            data = []
            gt = []
            test_list = '/media/home_bak/ziqi/park/CNN_single/dataset/val.txt'
            f = open(test_list)
            for line in f:
                line_data = line.strip('\n')
                line_gt = line_data.replace(
                    'pic_crop', 'point_crop').replace('.jpg', '_OA.txt')
                data.append(self.ds_dir + line_data)
                gt.append(self.ds_dir + line_gt)
            self.data = data
            self.gt = gt
    ...
    def __getitem__(self, item):
        ...
        return data, gt, mask, item, imgPath
