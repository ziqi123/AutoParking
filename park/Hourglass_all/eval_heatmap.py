...
for i, (data, gt, mask, item, imgPath, heatmaps_targets) in enumerate(test_loader):
    ...
    for k in range(len(imgPath)):
        ...
        point_path = os.path.join(
            "/media/home_bak/ziqi/park/Hourglass_all/point", f.strip('.jpg')+'.txt')
        ...
        img_back_path = os.path.join(
            "/media/home_bak/ziqi/park/Hourglass_all/test_img_back", f)
...
plt.ylabel('accuracy(%)')
plt.show()
plt.savefig("/media/home_bak/ziqi/park/Hourglass_all/accuracy3.png")
