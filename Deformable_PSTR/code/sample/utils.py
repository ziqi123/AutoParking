...
def resize_hm(heatmap, hm_size):
    ...
    heatmap = cv2.resize(heatmap.transpose(1, 2, 0),
                         hm_size, interpolation=cv2.INTER_CUBIC)
    ...
    for i in range(pafs.shape[0]):
        ...
        for x in range(0, img.shape[0], 8):
            for y in range(0, img.shape[1], 8):
                if len_paf[x, y] > 0.25:
                    img = cv2.arrowedLine(
                        img, (y, x), (int(y + 6*paf_x[x, y]), int(x + 6*paf_y[x, y])), colors[i], 1)
    cv2.imshow(name, img)
    cv2.waitKey()
    ...
    for i in range(pafs.shape[0]):
        ...
        for x in range(0, img.shape[0], 8):
            for y in range(0, img.shape[1], 8):
                # if len_paf[x,y] > 0.25:
                img = cv2.arrowedLine(
                    img, (y, x), (int(y + 6*paf_x[x, y]), int(x + 6*paf_y[x, y])), [0, 255, 255], 1)
    return img
    ...
    if target_type == 'gaussian':
        ...
        for joint_id in range(num_joints):
            ...
            if ul[0] >= heatmap_size[0] or ul[1] >= heatmap_size[1] \
                ...
                # if out-bounds, set target_weight = 0
            ...
            if joints_vis[joint_id, -1]:
                target[joint_id][:, :] = np.zeros(
                    (heatmap_size[1], heatmap_size[0]), dtype=np.float32)
