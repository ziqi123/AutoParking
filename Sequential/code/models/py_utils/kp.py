...
from .matcher2 import build_matcher2
...
class kp(nn.Module):
    def __init__(self,
        ...
        self.bbox_embed = MLP(hidden_dim, hidden_dim, 10, 3)
        ...
        x_interpolate = torch.linspace(-1.5, 1.5,
                                       x_res, requires_grad=False).unsqueeze(0)
        y_interpolate = torch.linspace(-1.5, 1.5, y_res,
                                       requires_grad=False).unsqueeze(0)  # [1, y_res]
    ...
    def _train(self, *xs, **kwargs):
        ...
        cls_b, cls_s, ign_cls_b, ign_cls_s, cls_sort, val_pslot, ign_pslot, ign_pslot_idx = self._make_mask(
            indices, l_b, l_s, targets, B, pslot)
        numpy1 = cls_b.numpy()
        numpy2 = cls_s.numpy()
        # print('numpy1,numpy2', numpy1, numpy2)
        tmp = np.ones([16, 10])
        j = 0
        for i in range(len(numpy1)):
            tmp[numpy1[i]][j] = numpy2[i]
            j = j+1
            if i+1 < len(numpy1) and numpy1[i+1] != numpy1[i]:
                while j < 10:
                    tmp[numpy1[i]][j] = numpy2[i]
                    j = j+1
                j = 0
        # print('tmp', tmp)
        a = torch.arange(start=0, end=16, step=1)
        a = a.repeat_interleave(10).long()
        b = torch.from_numpy(tmp)
        b = b.reshape(-1).long()
        # tmp = np.ones([16, 1])
        # j = 0
        # for i in range(len(numpy1)):
        #     tmp[numpy1[i]][j] = numpy2[i]
        #     j = j+1
        #     while i+1 < len(numpy1) and numpy1[i+1] == numpy1[i]:
        #         i = i+1
        #     if i+1 < len(numpy1):
        #         i = i+1
        #         j = 0
        ...
        # # print('tmp', tmp)
        # a = torch.arange(start=0, end=16, step=1)
        # a = a.repeat_interleave(1).long()
        # b = torch.from_numpy(tmp)
        # b = b.reshape(-1).long()

        src_boxes = bboxes[a, b]
        # print('cls_b,cls_s',cls_b,cls_s)
        # print('bboxes[cls_b,cls_s]',bboxes[cls_b,cls_s])
        src_boxes = src_boxes.reshape(B, -1, 10)

        # print('src_boxes',src_boxes.shape)

        # print('outputs_class', outputs_class[0].shape)
        # print('bboxes', bboxes.shape)

        # C_stacked = outputs_class.transpose(1, 2).detach(
        # ).cpu().numpy()  # [person per image * B, 17, num queries (for keypoint)]

        # print('C_stacked', C_stacked.shape)
        # mid_coord = bboxes
        # coord_holder = []
        # for b, C in enumerate(C_stacked):
        #     # print('b, C', b, C.shape)
        #     _, query_ind = linear_sum_assignment(-C)
        #     coord_holder.append(mid_coord[b, query_ind.tolist()])
        # matched_coord = torch.stack(coord_holder, dim=0).reshape(
        #     outputs_class.size(0), -1, 10)

        # indices = torch.LongTensor([0, 2, 4])
        # indices2 = torch.LongTensor([1, 3, 5])
        # x = torch.rand(2, 4, 6)
        # y = torch.index_select(x, 2, indices)
        # z = torch.index_select(x, 2, indices2)
        # print("y", y)
        # print('max Y', torch.max(y, 2))
        # print("z", z)
        # print('min Z', torch.min(z, 2))
        # a = torch.max(y, 2)[0]
        # b = torch.min(z, 2)[0]
        # w = torch.sub(a, b)
        # h = torch.add(a, b)
        # r = torch.div(torch.add(a, b), 2.0)
        # print('w', w)
        # print('h', h)
        # print('h', r)
        # exit()
        # w = torch.sub(a, b).unsqueeze(2)
        # h = torch.add(a, b).unsqueeze(2)
        # r = torch.div(torch.add(a, b), 2.0).unsqueeze(2)
        # print('w', w)
        # print('h', h)
        # print('h', r)
        # c = torch.cat((w, h, r), 2)
        ...
        indices = torch.LongTensor([0, 2, 4, 6])
        indices2 = torch.LongTensor([1, 3, 5, 7])
        bboxes1 = torch.index_select(
            src_boxes, 2, indices.to(self.mask.device))
        bboxes2 = torch.index_select(
            src_boxes, 2, indices2.to(self.mask.device))
        max_x = torch.max(bboxes1, 2)[0]
        max_y = torch.max(bboxes2, 2)[0]
        min_x = torch.min(bboxes1, 2)[0]
        min_y = torch.min(bboxes2, 2)[0]

        center_x = torch.div(torch.add(max_x, min_x), 2.0).unsqueeze(2)
        center_y = torch.div(torch.add(max_y, min_y), 2.0).unsqueeze(2)
        box_w = torch.sub(max_x, min_x).unsqueeze(2)
        box_h = torch.sub(max_y, min_y).unsqueeze(2)
        bbox = torch.cat((center_x, center_y, box_w, box_h), 2)
        ...
        person_per_image = src_boxes.size(1)
        ...
        # matcher2 = build_matcher2(1, 1, 3, B)
        # indices = matcher(out, targets)
        # l_b, l_s, pslot = self._get_src_permutation_idx(indices)
        # cls_b, cls_s, ign_cls_b, ign_cls_s, cls_sort, val_pslot, ign_pslot, ign_pslot_idx = self._make_mask(
        #     indices, l_b, l_s, targets, B, pslot)
        ...
        # [16, 4, 1]
        bbox = bbox.flatten(end_dim=1)

        # x1 = (x1 * 1.25 - 0.625) * \
        #     bbox[:, 2].unsqueeze(-1) + bbox[:, 0].unsqueeze(-1)
        # y1 = (y1 * 1.25 - 0.625) * \
        #     bbox[:, 3].unsqueeze(-1) + bbox[:, 1].unsqueeze(-1)
        ...
        out['pred_kpt_logits'] = j_class
        # if self.aux_loss:
        #     out['aux_outputs'] = self._set_aux_loss(
        #         j_coord, outputs_class)·
    ...
    def _test(self, *xs, **kwargs):
        ...
        indices2 = torch.LongTensor([1, 3, 5, 7])
        bboxes1 = torch.index_select(
            matched_coord, 2, indices.to(self.mask.device))
        bboxes2 = torch.index_select(
            matched_coord, 2, indices2.to(self.mask.device))
        max_x = torch.max(bboxes1, 2)[0]
        max_y = torch.max(bboxes2, 2)[0]
        min_x = torch.min(bboxes1, 2)[0]
        min_y = torch.min(bboxes2, 2)[0]
        ...
        # some preperation for STN feature cropping
        person_per_image = matched_coord.size(1)
        # print('person_per_image', person_per_image)
        # [100]
        num_person = person_per_image * matched_coord.size(0)
        # print('num_person', num_person)
        # [1600]
        heights, widths = sample.get_shape().unbind(-1)  # [B] * 2
        # print('sample.get_shape()', sample.get_shape().shape)
        ...
        # [800, 50, 3]
        j_coord = j_coord.reshape(
            matched_coord.size(0), -1, 10)

        j_class = j_class.reshape(matched_coord.size(
            0), -1, 3)
        # [16, 2500, 10]
        # [16, 2500, 3]
...
class AELoss(nn.Module):
    def __init__(self,
        ...
        losses = ['cls', 'joints', 'class']
        # losses = ['points', 'class']
        self.debug_path = debug_path
        self.matcher = build_matcher(1, 1, 3, bsize)
        self.matcher2 = build_matcher2(1, 1, 3, bsize*10)
        ...
        if save:
            ...
            if use_indices == False:
                ...
                with torch.no_grad():
                    ...
                    pred_classes = outputs['pred_kpt_logits']
