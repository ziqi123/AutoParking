...
def kp_detection(nnet, demo_dir, test_dir, decode_func=kp_decode, threshold=0.9, threshold_pck=3):
    ...
    dist = []
    list_pck = [0, 0]
    pred_point_dir = '/media/home_bak/ziqi/park/Ps_locate_dataset/PLD_BirdView_Training_TestSet_v1.0.7_All_3342/final_data/pred_point_256_deformable'
    if not os.path.exists(pred_point_dir):
        os.makedirs(pred_point_dir)
    # num_images = len(os.listdir(demo_dir))
    for ind in tqdm(sorted(os.listdir(os.path.join(demo_dir, 'data/img_256'))), ncols=67, desc="locating kps"):
        image_name = os.path.join(
            demo_dir, 'data/img_256', ind)
        orimage = cv2.imread(image_name)
        gth_name = os.path.join(demo_dir, 'data/annt_256',
                                ind.strip('.jpg')+'_OA.txt')
        # Parkgth = load_gth(gth_name)
        Parkgth = np.loadtxt(gth_name)
        Parkgth = Parkgth.reshape(1, 8)
        parking = np.zeros((2, 8), dtype=np.float)
        parking[1, :] = Parkgth

        parking_ignore = np.zeros((1, 8), dtype=np.float)

        Park = [parking, parking_ignore]

        image = cv2.resize(orimage, (256, 256))
        input = image.astype(np.float32) / 255.
        normalize_(input, mean, std)
        height, width = image.shape[0:2]
        inputs = np.zeros((1, 1, 3, 256, 256), dtype=np.float32)
        inmasks = np.zeros((1, 1, 1, 256, 256), dtype=np.float32)
        ...
        pred_joints = 256 * outputs['pred_boxes'].detach()
        ...
        TP, FP, TN, store, fail_joints, correct_joints, omit_joints, list_pck = test_pslot(
            Park, joints, TP, FP, TN, ind, list_pck, pred_point_dir, threshold_pck)
        if store:
            ori = deepcopy(image)
            orimage = deepcopy(image)
            gth = make_gth(orimage, Park)
    ...
    print('TN', TN)
    print('FP', FP)
    print('list_pck[0]', list_pck[0])
    print('list_pck[1]', list_pck[1])
...
def make_mask(pslot):
    ...
    im = np.zeros((256, 256), dtype="uint8")
...
def cal_distance(ps, ps_pred, list, ind, pred_point_dir, threshold=3):
    ps = ps.reshape((4, 2))
    ...
    dist = 0
    for i in range(2, 4):
        dist += ((int(ps[i][0])-int(ps_pred[i][0]))**2 +
                 (int(ps[i][1])-int(ps_pred[i][1]))**2)**0.5

    pred_point_path = os.path.join(
        pred_point_dir, ind.strip('.jpg')+'.txt')
    with open(pred_point_path, "w") as f:
        for i in range(2, 4):
            f.write(str(ps_pred[i][0]))
            f.write(' ')
            f.write(str(ps_pred[i][1]))
            f.write('\n')
        for j in range(2):
            f.write(str(ps_pred[j][0]))
            f.write(' ')
            f.write(str(ps_pred[j][1]))
            f.write('\n')
...
def test_pslot(all_pslot, pred, TP, FP, TN, ind, list_pck, pred_point_dir, threshold_pck=3, threshold=0.7, threshold_mask=0.7):
    pred_mask = []
    omit_joints = []
    if pred.shape[0] > 0:
            ...
            for idx, ps in enumerate(pslot_mask):
                ...
                if max(iou) > threshold:
                    ...
                    list_pck = cal_distance(
                        pslot[1+idx], pred[np.argmax(np.array(iou))][:8], list_pck, ind, pred_point_dir, threshold_pck)
                    TP += 1
                else:
...
def load_gth(annt):
    ...
    if count_ign != 0:
        for id in range(count_ign):
            parking_ignore[id + 1, :] = l_ign[id]/4
    if count != 0:
