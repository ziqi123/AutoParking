...
def kp_detection(nnet, demo_dir, test_dir, decode_func=kp_decode, threshold=0.9, threshold_pck=3):
    ...
    for ind in tqdm(sorted(os.listdir(os.path.join(demo_dir, 'img_1024'))), ncols=67, desc="locating kps"):
        image_name = os.path.join(
            demo_dir, 'img_1024', ind)
        orimage = cv2.imread(image_name)
        gth_name = os.path.join(demo_dir, 'annt_1024',
        ...
        # continue
        TP, FP, TN, store, fail_joints, correct_joints, omit_joints, list_pck, list_pck_1024 = test_pslot(
            Parkgth, joints, TP, FP, TN, ind, list_pck, list_pck_1024, threshold_pck)
        if store:
            ori = deepcopy(image)
    ...
    print('precision on {} pixels in 1024 img'.format(5),
          list_pck_1024[0]/list_pck_1024[1])
...
def cal_distance(ps, ps_pred, list, threshold=3):
    ps = ps.reshape((4, 2))
...
def test_pslot(all_pslot, pred, TP, FP, TN, ind, list_pck, list_pck_1024, threshold_pck=3, threshold=0.7, threshold_mask=0.7):
    pred_mask = []
    omit_joints = []
    if pred.shape[0] > 0:
        ...
        if len(pred_mask) == 0:
            ...
            for idx, ps in enumerate(pslot_mask):
                ...
                if max(iou) > threshold:
                    ...
                    list_pck_1024 = cal_distance(
                        pslot_1024[1+idx], pred_point[np.argmax(np.array(iou))][:8], list_pck_1024, threshold_pck)
                    TP += 1
                else:
    ...
    return TP, FP, TN, store, fail_joints, correct_joints, omit_joints, list_pck, list_pck_1024
