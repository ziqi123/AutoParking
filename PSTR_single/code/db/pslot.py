...
class PSLOT(DETECTION):
        ...
        for img_id in self._image_ids:
            ...
            for line in annt:
                ...
                if line_annt[-4] in ['1', '5']:
                    count_ign += 1
                    l_ign.append(
                        np.array([int(line_annt[i+1]) for i in range(8)]))
                    continue

            parking = np.zeros(
                (count+1, 8), dtype=np.float) if count != 0 else np.zeros((1, 8), dtype=np.float)
            parking_ignore = np.zeros(
                (count_ign+1, 8), dtype=np.float) if count_ign != 0 else np.zeros((1, 8), dtype=np.float)
            if count_ign != 0:
                for id in range(count_ign):
                    parking_ignore[id+1, :] = l_ign[id]
            if count != 0:
    ...
    def detections(self, ind):
        ...
        # ps_joints_ign = [[0 for i in range(8)] for i in range(1)]
        # ps_joints_ign = np.array(ps_joints_ign)

        return ps_joints.copy(), \
            ps_joints_ign.copy()

        # joints.astype(float).copy(), \
        # joints_vis.astype(float).copy()
