...
class kp(nn.Module):
    def __init__(self,
        ...
        self.class_embed2 = nn.Linear(hidden_dim, 2)  # 9 subclasses
        self.class_embed3 = nn.Linear(hidden_dim, 6)  # 9 subclasses
        self.joints_embed = MLP(hidden_dim, hidden_dim,
                                kps_dim, mlp_layers)  # 5 keypoints 5 * 2
        self.bbox_embed = MLP(hidden_dim, hidden_dim, 20, 3)
    ...
    def _train(self, *xs, **kwargs):
        ...
        # matcher = build_matcher2(1, 1, 3, B)
        ...
        # torch.Size([1600, 100, 10])
        # print('j_class', j_class.shape)

        # torch.Size([1600, 100, 3])
        # C_stacked = j_class.transpose(1, 2).detach().cpu().numpy()
        # coord_holder = []
        # for b, C in enumerate(C_stacked):
        #     _, query_ind = linear_sum_assignment(-C)
        #     coord_holder.append(j_coord[b, query_ind.tolist()])
        # pred_pslots = torch.stack(coord_holder, dim=0).reshape(
        #     j_coord.size(0), 5, -1)
        # print('pred_pslots', pred_pslots.shape)
        # exit()
...
class AELoss(nn.Module):
    def __init__(self,
        ...
        # self.matcher2 = build_matcher2(1, 1, 3, bsize)
    ...
    def forward(self,
        ...
        #     if k in weight_dict:
        #         print("k", k)
        #         print('loss_dict[k]', loss_dict[k].shape)
        #         print(' weight_dict[k]', weight_dict[k])
        ...
        if save:
            ...
            if use_indices == False:
                ...
                with torch.no_grad():
                    ...
                    # pred_pslots = pred_joints

                    # print('pred_pslots', pred_pslots[0].shape)
                    viz_tgt_joints = [
                        i.detach()*roi_h for i in targets[1:raw_batch+1]]
                    # print('viz_tgt_joints', viz_tgt_joints[0].shape)
