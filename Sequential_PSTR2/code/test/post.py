...
def NMS(param, heatmaps, upsampFactor=1., bool_refine_center=True, bool_gaussian_filt=False):
    ...
    # print("upsampFactor: {}".format(upsampFactor))
    ...
    for joint in range(NUM_JOINTS):
        map_orig = heatmaps[:, :, joint] # 747 1407
        peak_coords = find_peaks(param, map_orig)

        peaks = np.zeros((len(peak_coords), 4))
        for i, peak in enumerate(peak_coords):
            if bool_refine_center:
                x_min, y_min = np.maximum(0, peak - win_size) # -2 -2
                x_max, y_max = np.minimum(np.array(map_orig.T.shape) - 1, peak + win_size) # +2 +2
                ...
                # print(map_upsamp.shape) # 5 x upsampFactor, 5  x upsampFactor
                ...
                # print(len(location_of_max))
                # print(location_of_max) # coord in map_upsamp
                ...
                # print(location_of_patch_center) center of patch
                # print(peak[::-1]) # x,y -> y, x
...
def find_connected_joints(param, paf_upsamp, joint_list_per_joint_type, num_intermed_pts=10,
                          penaliz_factor=1.5, pos_upper_bound=0.):
    """
    ...
    # print("limb_intermed_coords.shape: {}".format(limb_intermed_coords.shape)) # 4, 10
    # print("limb_intermed_coords:\n{}".format(limb_intermed_coords)) # all zeros

    for limb_type in range(NUM_LIMBS):
        # print("limb_type: {}".format(limb_type))
        # List of all joints of type A found, where A is specified by limb_type
        # (eg: a right forearm starts in a right elbow)

        sort_start_bottom = []
        joints_src_ori = joint_list_per_joint_type[joint_to_limb_heatmap_relationship[limb_type][0]]
        # print("joints_src_ori:\n{}".format(joints_src_ori))
        # print(joints_src_ori.shape) # num, 4

        # this part ugly
        for k in range(joints_src_ori.shape[0]):
            for j in range(joints_src_ori.shape[0] - k - 1):
                if joints_src_ori[j, 1] > joints_src_ori[j + 1, 1]:
                    joints_src_ori[j], joints_src_ori[j + 1] = joints_src_ori[j + 1], joints_src_ori[j]
        for k in range(joints_src_ori.shape[0]):
            sort_start_bottom.append(joints_src_ori[-(k+1)])


        joints_src = np.stack(sort_start_bottom, axis=0)
        # print("joints_src:\n{}".format(joints_src))
        ...
        if len(joints_src) == 0 or len(joints_dst) == 0:
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    # print("joint_dst[:2]: {}".format(joint_dst[:2]))
                    # print("joint_src[:2]: {}".format(joint_src[:2]))
                    ...
                    limb_intermed_coords[1, :] = np.round(np.linspace(
                        joint_src[0], joint_dst[0], num=num_intermed_pts)) # x coordinate
                    ...
                    # print("intermed_paf: {}\n{}".format(intermed_paf.shape, intermed_paf)) # 10, 2

                    score_intermed_pts = intermed_paf.dot(limb_dir)
                    # print("score_intermed_pts: {}\n{}".format(score_intermed_pts.shape, score_intermed_pts)) # 10

                    score_penalizing_long_dist = score_intermed_pts.mean() + \
                                                 min(1 / penaliz_factor * paf_upsamp.shape[0] / limb_dist - 1,
                                                     pos_upper_bound)
                    # print("score_penalizing_long_dist: {}".format(score_penalizing_long_dist)) # scalar score
                    ...
                    # print("criterion1: {}".format(criterion1))
                    ...
                    # print("criterion2: {}".format(criterion2))

                    if criterion1 and criterion2:
                        src_candidate.append([
                            limb_dist,
                ...
                if len(src_candidate): # find candidate
                    min_dst_candidate = src_candidate[0]
                    if len(src_candidate) > 1:
                        for l in range(len(src_candidate))[1:]:
                            if src_candidate[l][0] < min_dst_candidate[0]:
                                min_dst_candidate = src_candidate[l]

                    connection_candidates.append(min_dst_candidate[1:])
                    used_dst.append(min_dst_candidate[2])


            # for l in range(len(connection_candidates)):
            #     print(connection_candidates[l])
            ...
            # # print("connection_candidates:\n{}".format(connection_candidates)) # sorted by score, all limb in img
            # for l in range(len(connection_candidates)):
            #     print(connection_candidates[l])
            ...
            # print("max_connections: {}".format(max_connections))
...
def group_limbs_of_same_person(connected_limbs, joint_list):
    ...
    for limb_type in range(NUM_LIMBS):
        ...
        # print("joint_src_type: {}".format(joint_src_type))
        # print("joint_dst_dtype: {}".format(joint_dst_type))
        ...
        for limb_info in connected_limbs[limb_type]:
            ...
            # print("person_assoc_idx:\n{}".format(person_assoc_idx))
            # print("person_to_joint_assoc:\n{}".format(person_to_joint_assoc))
    ...
    for person_id, person_info in enumerate(person_to_joint_assoc):
        # print("person_info: {}".format(person_info))
        # print("person_info[-1]: {}".format(person_info[-1]))
        # print("person_info[-2] / person_info[-1]: {}".format(person_info[-2] / person_info[-1]))
    ...
    # print("person_to_joint_assoc: {}".format(person_to_joint_assoc))
...
def decode_pose(img_orig, param, heatmaps, pafs):
    ...
    # [1]=neck...)
    # print("img_orig.shape: {}".format(img_orig.shape)) # 767 1407 3
    # print("heatmaps.shape: {}".format(heatmaps.shape)) # 767 1407 2

    joint_list_per_joint_type = NMS(param, heatmaps, img_orig.shape[0] / float(heatmaps.shape[0]))
    # print(type(joint_list_per_joint_type)) # list
    # print(len(joint_list_per_joint_type)) # 2
    # for i in range(len(joint_list_per_joint_type)):
    #     print(type(joint_list_per_joint_type[i])) # np array
    #     print(joint_list_per_joint_type[i].shape) # num 4
    # for joint_type, joint_peaks in enumerate(joint_list_per_joint_type):
    #     print("joint_type: {}".format(joint_type)) # 0 or 1 (tl, br)
    #     print("type(joint_peaks): {}".format(type(joint_peaks))) # np array
    #     print("joints_peaks.shape: {}".format(joint_peaks.shape)) # num 4
    #     for peak in joint_peaks:
    #         print(peak) # x, y, score, idx
    # exit(1)
    ...
    joint_list = np.array([tuple(peak) + (joint_type,)
                           for joint_type, joint_peaks in enumerate(joint_list_per_joint_type)
                           for peak in joint_peaks])
    # print("joint_list.shape: {}".format(joint_list.shape)) # num of (tl + br), 5 | x, y, score, idx, type
    # print("joint_list:\n{}".format(joint_list))
    ...
    # print("paf_upsamp.shape: {}".format(paf_upsamp.shape)) # 767 1407
    ...
    # print(person_to_joint_assoc.shape) # 5, 4 |
...
def append_result(person_to_joint_assoc, joint_list, offset, input_width, input_height, new_height, new_width):
    ...
    # out_path = os.path.join(paf_out_dir, os.path.basename(image_id[:-4]) + '.txt')
    # out_file = open(out_path, 'w')
    # print("joint_list: {}".format(joint_list))
    out_keypoints = []
    for ridxPred in range(len(person_to_joint_assoc)):
        # print("ridxPred: {}".format(ridxPred))
        total_score = person_to_joint_assoc[ridxPred, 2]

        keypoints = np.zeros((2, 4))
        for part in range(2):
            ind = part
            ...
            if -1 == index:
                ...
                keypoints[part, 0] = min(input_width - 1, max(0, joint_list[index, 0] + 0.5 + offset[1]))  # x
                keypoints[part, 0] *= new_width / input_width
                # print(new_width, input_width)  # 1280 1407
                # print(new_height, input_height)  # 720 767
                keypoints[part, 1] = min(input_height - 1, max(0, joint_list[index, 1] + 0.5 + offset[0]))  # y
                keypoints[part, 1] *= new_height / input_height
                keypoints[part, 2] = joint_list[index, 2]
                keypoints[part, 3] = total_score
        out_keypoints.append(keypoints)

    return out_keypoints


    #     out_file.write('car' + ' ')
    #     out_file.write(str((keypoints[0, 2] + keypoints[1, 2]) / 2.) + ' ')
    #     out_file.write(str(keypoints[0, 0]) + ' ')
    #     out_file.write(str(keypoints[0, 1]) + ' ')
    #     out_file.write(str(keypoints[1, 0]) + ' ')
    #     out_file.write(str(keypoints[1, 1]) + ' ')
    #     out_file.write('\n')
    #     cv2.rectangle(source_image,
    #                   (int(keypoints[0, 0]), int(keypoints[0, 1])),
    #                   (int(keypoints[1, 0]), int(keypoints[1, 1])), [0, 255, 0], 2)
    #
    # cv2.imwrite(os.path.join(paf_debug_dir, os.path.basename(image_id[:-4]) + '.jpg'), source_image)

def draw_keypoints(source_image, keypoints):
    num = len(keypoints)
    if num:
        for i in range(num):
            keypoint = keypoints[i]
            cv2.rectangle(source_image,
                          (int(keypoint[0, 0]), int(keypoint[0, 1])),
                          (int(keypoint[1, 0]), int(keypoint[1, 1])),
                          [0, 255, 0], 2)
    return source_image
    # cv2.imwrite(os.path.join(paf_debug_dir, os.path.basename(image_id[:-4]) + '.jpg'), source_image)
