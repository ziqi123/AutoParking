...
class SetCriterion(nn.Module):
    ...
    def loss_joints(self, outputs, targets, indices, num_pslots):
        ...
        # src_boxes2 = outputs['pred_boxes'][cls_b, cls_s]
        # src_ign_boxes2 = outputs['pred_boxes'][ign_cls_b, ign_cls_s]
        ...
        # cost_points2 = torch.cdist(src2, tgt, p=1)
        C = cost_points.view(num_pslot, 4, -1).cpu()
        # C2 = cost_points2.view(num_pslot, 4, -1).cpu()
        ...
        # src_ign2 = src_ign_boxes2[:, :8].view(-1, 4, 2).flatten(0, 1).detach()
        cost_points_ign = torch.cdist(src_ign, tgt_ign, p=1)
        # cost_points_ign2 = torch.cdist(src_ign2, tgt_ign, p=1)
        C = cost_points_ign.view(num_pslot_ign, 4, -1).cpu()
        # C2 = cost_points_ign2.view(num_pslot_ign, 4, -1).cpu()
        ...
        # loss_bbox2 = F.l1_loss(src_boxes2, tgt_boxes, reduction='none')
        loss_bbox_ign = F.l1_loss(
            src_ign_boxes, tgt_ign_boxes, reduction='none')
        # loss_bbox_ign2 = F.l1_loss(
        #     src_ign_boxes2, tgt_ign_boxes, reduction='none')
        losses = {}
        losses['loss_boxes'] = (
    ...
    def forward(self, outputs, targets):
        ...
        for loss in self.losses:
            losses.update(self.get_loss(
                loss, outputs, targets, indices, num_pslots))
