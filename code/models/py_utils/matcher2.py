...
#         out_prob = out_prob.repeat(1, 5)
#         out_prob = out_prob.reshape(-1, 3)
...
#         # tgt_ids = tgt_ids.repeat_interleave(5)
#         # print('tgt_ids', tgt_ids.shape)
#         # tgt_ids = tgt_ids.repeat(1, 5)
#         # print('tgt_ids', tgt_ids.shape)
...
#         # tgt_bbox = torch.repeat_interleave(tgt_bbox, 5, 0)
...
#         sizes = [val for val in sizes for i in range(5)]
#         print('sizes.sum', sum(sizes))
#         print('C.split(sizes, -1)', C.split(sizes, -1)[0].shape)
#         print('C.split(sizes, -1)', C.split(sizes, -1)[1].shape)
#         # 1475
...
# def build_matcher2(set_cost_class, set_cost_bbox, set_cost_giou, batch_size):
    ...
    def forward(self, outputs, targets):
        ...
        Returns:
            ...
            For each batch element, it holds:
                len(index_i) = len(index_j) = min(num_queries, num_parking_slots)
        """
        indices0 = torch.LongTensor([0, 1, 2, 3])
