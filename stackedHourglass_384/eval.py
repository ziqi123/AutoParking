...
import cv2
import numpy as np
from dataloader.dataloader_stackedHourglass_384 import heatmap_Dataloader
import os
from models.posenet_384 import PoseNet
...
model.load_state_dict(torch.load(
    '/media/home_bak/ziqi/park/stackedHourglass_384/10heatmap.ckpt'))
...
def get_acc(path, y, y_hat, dis):
    ...
    file = os.path.join(markImg_384_dir, f)
...
def cvArrow(img, pt1, pt2, img_path):
...
if __name__ == "__main__":
    ...
    # 384*384数据集
    img_384_dir = "/media/home_bak/ziqi/park/Ps_locate_dataset/PLD_BirdView_Training_TestSet_v1.0.7_All_3342/perspective_img"

    # 保存最终标记上预测点和方向的384*384图片
    markImg_384_dir = "./mark_test_img_384"
    # 192*192上的真值
    gt_192_gt_dir = "/media/home_bak/ziqi/park/Ps_locate_dataset/PLD_BirdView_Training_TestSet_v1.0.7_All_3342/match_img_point"
    ...
    # # Forward pass
        outputs = model(data)
    # h, w = 48, 48
    # kpts = get_kpts(outputs[0], img_h=h, img_w=w)
    # print(kpts)
    # kpts = get_kpts(outputs[2], img_h=h, img_w=w)
    # print(kpts)
    # kpts = get_kpts(outputs[-1], img_h=h, img_w=w)
    # print(kpts)
    ...
    # # 热图还原为坐标点
        all_peak_points = 4*get_peak_points(outputs[0].cpu().data.numpy())
...
#     # # 标点
        for k in range(len(imgPath)):
            ...
            gt_192_path = os.path.join(
                gt_192_gt_dir, f.strip('.jpg')+'_OA.txt')
            point_384_gt_path = os.path.join(
                point_384_gt_dir, f.strip('.jpg')+'.txt')
        # 在192*192图片上标记预测点和真值点
            colorize(imgPath[k], markImg_384_path, all_peak_points[k], gt[k])
        # 将192*192上的预测点和真值还原到384*384上，并在384*384图片上标点
