...
from models.layers2 import Conv, Hourglass, Pool, Residual
from models.loss import HeatmapLoss


class UnFlatten(nn.Module):
    def forward(self, input):
        return input.view(-1, 192, 4, 4)
...
class PoseNet(nn.Module):
    def __init__(self, nstack=8, inp_dim=256, oup_dim=4, bn=False, increase=0, **kwargs):
    ...
    def calc_loss(self, combined_hm_preds, heatmaps):
        combined_loss = []
        for i in range(self.nstack):
            combined_loss.append(self.heatmapLoss(
                combined_hm_preds[0][:, i], heatmaps))
