...
class heatmap_dataset(Dataset):
    def __init__(self, ds_dir, sigma, setname='train', transform=None, norm_factor=256, rgb2gray=False, resize=True):
        ...
        if setname == 'train':
            data = []
            gt = []
            train_list = '/media/home_bak/ziqi/park/stackedHourglass_384/dataset/train.txt'
            f = open(train_list)
            for line in f:
        ...
        if setname == 'val':
            data = []
            gt = []
            test_list = '/media/home_bak/ziqi/park/stackedHourglass_384/dataset/val.txt'
            f = open(test_list)
            for line in f:
                line_data = line.strip('\n')
                line_gt = line_data.replace(
    ...
    def __getitem__(self, item):
        ...
        generateHeatmap = GenerateHeatmap(96, 4)
        gt2 = gt/384*96
